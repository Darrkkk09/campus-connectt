...
@router.post("/get_questions_from_llm")
async def get_questions_from_llm(
    role: str = Form(...),
    ...
    Output: A list of 10 technical Questions relevant to the role without question numbers and without any other text.
    """

    try:
        model = genai.Model(gemini_mode)
        response = model.generate_content(prompt)
        text = response.text.strip()
        lines = [q.strip() for q in text.split('\n') if q.strip()]

        result = [{"id": i + 1, "question": q} for i, q in enumerate(lines)]
